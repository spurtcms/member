...
func (member *Member) CheckProfileSlug(profileSlug string, profileID int) (TblMemberProfile, error) {
	...
	profile, err := Membermodel.CheckProfileSlug(profileSlug, member.DB)
...
func (member *Member) DashboardMemberCount() (totalcount int, lasttendayscount int, err error) {

	if AuthErr := AuthandPermission(member); AuthErr != nil {
		return 0, 0, AuthErr
	}

	allmembercount, err := Membermodel.AllMemberCount(member.DB)
	if err != nil {
		return 0, 0, err
	}

	Lmembercount, err := Membermodel.NewmemberCount(member.DB)
	if err != nil {
		return 0, 0, err
	}
...
func (member *Member) ActiveMemberList(limit int) (memberdata []Tblmember, err error) {
	...
	var members []Tblmember
	activememlist, err := Membermodel.ActiveMemberList(members, limit, member.DB)
...
func (member *Member) MemberFlexibleUpdate(memberData map[string]interface{}, memberId, modifiedBy int) error {
	...
	err := Membermodel.FlexibleMemberUpdate(memberData, memberId, member.DB)
	if err != nil {
		return err
	}
...
func (member *Member) MemberProfileFlexibleUpdate(memberProfileData map[string]interface{}, memberId, modifiedBy int) error {
	...
	err := Membermodel.FlexibleMemberProfileUpdate(memberProfileData, memberId, member.DB)
	if err != nil {
		return err
	}
...
func (member *Member) MemberPasswordUpdate(newPassword, confirmPassword, oldPassword string, memberId, modifiedBy int) error {
	...
	var memberData TblMember
	if err := Membermodel.GetMemberDetailsByMemberId(&memberData, memberId, member.DB); err != nil {
		return err
	}
	...
	err := Membermodel.MemberPasswordUpdate(memberData, memberId, member.DB)
	if err != nil {
		return err
	}
